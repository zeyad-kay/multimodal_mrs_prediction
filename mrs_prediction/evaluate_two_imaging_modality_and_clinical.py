...
def validate(model, data_loader, device, tasks, metrics, bootstrap_rounds, ci, random_state):
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            tabular = labels[:,1:]
            labels = labels[:,0:1]
            outputs = model(batch["ncct"].to(device), batch["cta"].to(device), tabular.to(device))
...
def main(args):
    ...
    test_data = [{"ncct": ncct, "cta": cta, "labels": labels} for ncct, cta, *labels in test_df[["B0_NCCT","B0_CTA"] + targets + tabular].values]
