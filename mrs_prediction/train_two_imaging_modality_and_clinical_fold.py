...
def train(model, criterion, criterion_weights, optimizer, data_loader, rank, log_dict):
    ...
    for batch in data_loader:
        ...
        tabular = labels[:,1:]
        labels = labels[:,0:1]
        outputs = model(batch["ncct"].cuda(), batch["cta"].cuda(), tabular.cuda())
...
def main(args):
    ...
    train_data = [{"ncct": ncct, "cta": cta, "labels": labels} for ncct, cta, *labels in train_df[["B0_NCCT","B0_CTA"] + targets + tabular].values]
    ...
    if global_rank == 0:
        ...
        val_data = [{"ncct": ncct, "cta": cta, "labels": labels} for ncct, cta, *labels in val_df[["B0_NCCT","B0_CTA"] + targets + tabular].values]
