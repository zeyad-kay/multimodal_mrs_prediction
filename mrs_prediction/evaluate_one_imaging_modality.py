...
def validate(model, data_loader, device, tasks, metrics, bootstrap_rounds, ci, random_state):
    model.eval()
    true = torch.empty((len(data_loader.dataset), len(tasks)), device=device)
    logits = torch.empty((len(data_loader.dataset), len(tasks)), device=device)
    file_paths = []
    idx = 0
    with torch.no_grad():
...
def main(args):
    ...
    test_loader = get_dataloader(test_df, modality, targets, batch_size=batch_size, transform=test_transform, num_workers=2)
